...
function EditPatient() {
  ...
  const handleInputChange = (e) => {
    ...
    setPatient((prev) => ({
      ...prev,
      [name]: selectedDate.toISOString().split("T")[0], // Store YYYY-MM-DD
    }));
  } else {
  ...
  const fetchMedicineSuggestions = async (query, type) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/medicines/search?query=${query}`);
  ...
  const addMedicine = async (selectedMedicine, type) => {
    ...
    try {
      ...
      if (selectedMedicine.id) {
        ...
        const checkRes = await fetch(`http://localhost:5000/medicines/search?query=${selectedMedicine.name}`);
 ...
 required />
            ...
            <div>
              ...
              <input type="date" name="discharge_date" value={patient.discharge_date} onChange={handleInputChange} max={new Date().toLocaleDateString("en-CA")}
